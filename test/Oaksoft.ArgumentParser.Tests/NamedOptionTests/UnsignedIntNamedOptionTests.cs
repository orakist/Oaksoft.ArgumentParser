...
public class UnsignedIntNamedOptionTests : ArgumentParserTestBase
{
    [Theory]
    ...
    public void ParseScalarOption_WhenArgumentsValid_WithDefaultValues(uint val1, uint val2, params string[] args)
    {
        // Arrange
        var sut = CommandLine.CreateParser<UnsignedIntAppOptions>()
            .AddNamedOption(s => s.Value, o => o.WithDefaultValue((uint)11), mustHaveOneValue: false)
            .AddNamedOption(s => s.NullValue, o => o.WithDefaultValue((uint)55), mustHaveOneValue: false)
            .Build();
